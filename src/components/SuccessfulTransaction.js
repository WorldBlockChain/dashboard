...
export default class TransactionsChart extends React.Component {
  ...
  onNewLedger(ledger) {
    ...
    data[1].values.push({x: ledger.sequence.toString(), y: ledger.failed_transaction_count});
    data[0].values.shift();
    data[1].values.shift();
    ...
    axios.get(this.url)
      .then(response => {
        ...
        each(response.data._embedded.records, ledger => {
          ...
          data[1].values.unshift({x: ledger.sequence.toString(), y: ledger.failed_transaction_count});
        });
        this.setState({loading: false, data});
      ...
      <div ref={(el) => { this.panel = el; }}>
        <Panel>
          <div className="widget-name transactionledger">
            ...
            </span> &amp; <span style={{borderBottom: '2px solid #FF6F00'}}>FAILED</span> TXS in the last {this.props.limit} ledgers
{/*
